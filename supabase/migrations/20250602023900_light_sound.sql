-- Remove the deadline check from the settle_crypto_wager function to allow early settlement
-- First drop any existing function with the same name and parameter types
DROP FUNCTION IF EXISTS settle_crypto_wager(UUID, TEXT, DOUBLE PRECISION);

-- Create the updated settle_crypto_wager function that allows early settlement
...
BEGIN
  ...
  -- No longer checking if deadline has passed - this allows early settlement
  -- if target price is met before the deadline
...
COMMENT ON FUNCTION settle_crypto_wager(UUID, TEXT, DOUBLE PRECISION) IS 'Function to settle a crypto wager with price verification, allowing early settlement if target price is met';