/*
  # Create sports_wagers table for WagerFi app

  1. New Tables
    - `sports_wagers`
      - `id` (uuid, primary key) - Unique identifier for the sports wager
      - `creator_address` (text) - Wallet address of the user creating the wager
      - `opponent_address` (text) - Wallet address of the user accepting the wager (nullable at first)
      - `creator_team_id` (integer) - Team ID selected by creator
      - `opponent_team_id` (integer) - Team ID selected by opponent (optional)
      - `sol_amount` (double precision) - Stake amount per user
      - `event_id` (integer) - Unique match/fixture ID from the sports API
      - `sport_type` (text) - Type of sport (soccer, nba, ufc, etc.)
      - `status` (text) - Status of the wager (open, live, settled, cancelled)
      - `expires_at` (timestamptz) - Cutoff time (usually match kickoff time)
      - `event_date` (date) - Date of the sporting event
      - `event_time` (time) - Time of the sporting event
      ...
      - `winner_address` (text) - Wallet address of the winner
  
  2. Security
    - Enable RLS on `sports_wagers` table
    - Add policies for viewing, creating, and updating sports wagers
    - Add constraints and validations
*/

-- Create the sports_wagers table
...
-- Function to accept a sports wager
...
-- Function to settle a sports wager
...
-- Function to cancel a sports wager
...
-- Function to get user sports wager statistics
