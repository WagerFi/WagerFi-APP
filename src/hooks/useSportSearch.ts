...
export const useSportSearch = ({ sportId }: UseSportSearchProps) => {
  ...
  // Get API endpoint and key based on selected sport - kept for backward compatibility
  const getApiConfig = useCallback((): ApiConfig => {
    // Use the same API key for all sports
    const apiKey = import.meta.env.VITE_FOOTBALL_API_KEY;
    ...
    console.log(`Using API for ${sportId}: ${baseUrl} with key available: ${!!apiKey}`);
  ...
  useEffect(() => {
    ...
    const debounceSearch = setTimeout(async () => {
      ...
      try {
        ...
        // Make the API call
        const response = await fetch(`${baseUrl}/${endpoint}?search=${encodeURIComponent(searchQuery)}`, {
          headers: {
  ...
  }, [searchQuery, sportId, decodeHtmlEntities, getApiConfig, isVsSearch]);
